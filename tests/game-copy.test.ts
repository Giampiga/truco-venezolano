...
import {
  ...
  actionLabels,
  pendingCanto,
} from '../lib/game-copy.ts';
void test('player messages use second person and Spanish action names', () => {
  ...
  assert.equal(
    gameEventText('human jugó 7 de oros.', names, 'human'),
    'Jugaste 7 de oros.',
  );
  assert.equal(
    gameEventText('human se fue al mazo.', names, 'human'),
    'Te fuiste al mazo.',
  );
  assert.equal(
    gameEventText('human pasó sus tres cartas.', names, 'human'),
    'Pasaste tus tres cartas.',
  );
  assert.equal(
    gameEventText('opponent jugó 7 de oros.', names, 'human'),
    'Truquito jugó 7 de oros.',
  );
  assert.equal(actionLabels['raise-envido'], 'Subir el envite');
});

void test('pending canto explains the active wager and suspended truco', () => {
  const state = {
    handComplete: false,
    priority: { active: 'envido', suspendedTruco: { call: 'truco', by: 'A' } },
    truco: {
      accepted: 'none',
      lastRaisedBy: null,
      pending: { call: 'truco', by: 'A' },
    },
    envido: {
      status: 'pending',
      acceptedStake: 2,
