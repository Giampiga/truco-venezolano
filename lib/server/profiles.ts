...
export async function profileRoom(room: StoredRoom, userId: string) {
  ...
  const rows = await getDb().prepare(`SELECT user_id, handle, name FROM profiles WHERE user_id IN (${room.members.map(() => '?').join(',')})`).bind(...room.members.map(m => m.userId)).all<{ user_id: string; handle: string; name: string }>();
